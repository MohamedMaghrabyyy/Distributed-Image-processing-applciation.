...
class MainServer(threading.Thread):
    ...
    def run(self):
        # while True:
            try:
                ...
                logging.info(f"[Master 2]: Received image with operation:{operation} ")
                processed = None
                if operation == "Gray":
                ...
                logging.error("[Master 2]: OSError")
    ...
    def receive_data(self):
        ...
        if ready_to_read:
            ...
            logging.info(f"[Master 2]: start receiving data")
        ...
        logging.info("[Master 2]: returning received array")
        return array

    def send_data(self, data):
        serialized_data = pickle.dumps(data)
        print("sending serialized")
        logging.info("[Master 2]: sending serialized")
...
def listenServer():
    ...
    while tcpSocket:
        ...
        if serverAddress[0] == "168.63.129.16":
            ...
            logging.info(f"[Master 2]: Health Probe")
            serverSocket.close()
            continue
        ...
        logging.info(f"[Master 2]: Starting Thread with:{serverAddress[0]} : {serverAddress[1]} ")
        newThread.start()
