...
class VMThread(threading.Thread):
    ...
    def process_image(self,ctx,queue, op, img, value, height = 0, width = 0):
        ...
        if op == 'intensity':
            processedChannel = gp3.apply_intensity_kernel_G(ctx,queue, img, value, self.kernels['bright'], self.kernels['dark'])
            return processedChannel
        elif op == "to_grey":
            processedChannel = gp3.process_grey_G(ctx,queue, img, height, width, self.kernels['grey'])
            return processedChannel
