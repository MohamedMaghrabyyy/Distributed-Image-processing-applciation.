...
class VMThread(threading.Thread):
    ...
    def process_image(self,ctx,queue, op, img, value, height = 0, width = 0):
        ...
        if op == 'intensity':
            processedChannel = gp2.apply_intensity_kernel_B(ctx,queue, img, value, self.kernels['bright'], self.kernels['dark'])
            return processedChannel
        elif op == "to_grey":
            processedChannel = gp2.process_grey_B(ctx,queue, img, height, width, self.kernels['grey'])
            return processedChannel
        elif op== "intensity-grey":
            processedChannel = gp2.apply_intensity_kernel_grey(ctx,queue, img,value, self.kernels['bright'], self.kernels['dark'])
            return processedChannel
